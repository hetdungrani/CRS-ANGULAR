    ...
    <div class="fixed top-4 right-4 z-[100] flex flex-col gap-3 w-full max-w-sm pointer-events-none">
      @for (toast of toastService.toasts(); track toast.id) {
        <div 
          ...
          [ngClass]="{
            'bg-white/90 border-green-200 text-green-800 dark:bg-gray-800/90 dark:border-green-800 dark:text-green-100': toast.type === 'success',
            'bg-white/90 border-red-200 text-red-800 dark:bg-gray-800/90 dark:border-red-800 dark:text-red-100': toast.type === 'error',
            'bg-white/90 border-yellow-200 text-yellow-800 dark:bg-gray-800/90 dark:border-yellow-800 dark:text-yellow-100': toast.type === 'warning',
            'bg-white/90 border-blue-200 text-blue-800 dark:bg-gray-800/90 dark:border-blue-800 dark:text-blue-100': toast.type === 'info'
          }"
        >
          <!-- Icon -->
          ...
          <div class="absolute bottom-0 left-0 h-1 bg-current opacity-20 w-full animate-shrink-width" [style.animation-duration.ms]="toast.duration || 3000"></div>
        </div>
      }
    </div>
  `,
  ...
  `],
    animations: [
        trigger('toastAnimation', [
            ...
            transition(':leave', [
                animate('300ms ease-in', style({ transform: 'translateX(100%) scale(0.8)', opacity: 0 }))
            ])
        ])
    ]
})
